...
# 1.0 -> 2.0 mejora de pantallas
# 2.0 -> 3.0 mejora funcion show displ
...
p_project = "Test de pantallas de riego"
p_version = "3.0"
...
def ShowDispL3Bif(lt, l1, l2, l3b, ls, Erase = True, Show = True):
...
tstamp = ticks_ms()
ShowDispL3Bif(Topline, 'OK: Disp...', '', 'HW Init Run', 'Init progress')
print('Tiempo =',ticks_diff(ticks_ms(),tstamp))
sleep(1)
tstamp = ticks_ms()
ShowDispL3Bif('', 'OK: Dis/3sw', '', '', 'Init %..', Erase = False)
print('Tiempo =',ticks_diff(ticks_ms(),tstamp))
sleep(1)

tstamp = ticks_ms()
ShowDispL3Bif('', 'OK: Dis/3sw/Pump', '', '', 'Init %....', Erase = False)
print('Tiempo =',ticks_diff(ticks_ms(),tstamp))
sleep(1)

tstamp = ticks_ms()
ShowDispL3Bif('', '', 'RE/', '', 'Init %......', Erase = False)
print('Tiempo =',ticks_diff(ticks_ms(),tstamp))
sleep(1)

tstamp = ticks_ms()
ShowDispL3Bif('', '', 'RE/SHum', '', 'Init %........', Erase = False)
print('Tiempo =',ticks_diff(ticks_ms(),tstamp))
sleep(1)
# 
# ShowDispL3B(Topline,'OK:Dis/3sw/Pump', 'RE/SHum/Tank NOK', 'FILL TANK' , 'HW INIT Fail')
tstamp = ticks_ms()
ShowDispL3Bif('','', 'RE/SHum/Tank OK', 'press CON->' , 'HW INIT END', Erase = False )
print('Tiempo =',ticks_diff(ticks_ms(),tstamp))
